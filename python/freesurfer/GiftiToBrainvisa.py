...
# Brainvisa Function
def GiftiToBrainvisa(srcmesh, dstmesh):
    _,mesh,texture = readGifti(srcmesh)
    if mesh != None:
      soma.aims.Writer().write(mesh, dstmesh)
    if texture!=None:
      soma.write(texture, dstmesh)


if __name__ == '__main__':
  if len(sys.argv) == 3:
    srcmesh, dstmesh = sys.argv[1:]
    _,mesh,texture = readGifti(srcmesh)
    if mesh != None:
      soma.aims.Writer().write(mesh, dstmesh)
    if texture!=None:
