...
def execution(self, context):
    ...
    context.write('%s -c %s f(\"%s\", \"%s\", \"%s\");'%(os.path.basename(sys.executable), cmd, self.PialMesh.fullPath(), self.bv_anat.fullPath(), self.AimsNativePial.fullPath()))
    context.pythonSystem('-c', '%s f(\"%s\", \"%s\", \"%s\");'%(cmd, self.PialMesh.fullPath(), self.bv_anat.fullPath(), self.AimsNativePial.fullPath()))
    
    context.write('%s -c %s f(\"%s\", \"%s\", \"%s\");'%(os.path.basename(sys.executable), cmd, self.WhiteMesh.fullPath(), self.bv_anat.fullPath(), self.AimsNativeWhite.fullPath()))
    context.pythonSystem('-c', '%s f(\"%s\", \"%s\", \"%s\");'%(cmd, self.WhiteMesh.fullPath(), self.bv_anat.fullPath(), self.AimsNativeWhite.fullPath()))
