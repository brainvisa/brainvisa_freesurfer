...
def execution(self, context):
    if self.SphereRegGifti is not None and self.SphereReg is None:
        context.warning(
            'SphereReg parameter is not set: SphereRegGifti will not be written.')
    context.write('Convert meshes in Freesurfer fromat to Gifti format.')
    context.write('mris_convert %s %s' %
                  (self.Pial.fullPath(), self.PialGifti.fullPath()))
    database = ''
    launchFreesurferCommand(
        context, database, 'mris_convert', self.Pial.fullPath(), self.PialGifti.fullPath())
    context.write('mris_convert %s %s' %
                  (self.White.fullPath(), self.WhiteGifti.fullPath()))
    launchFreesurferCommand(
        context, database,  'mris_convert', self.White.fullPath(), self.WhiteGifti.fullPath())
    ...
    if self.SphereReg is not None and self.SphereRegGifti is not None:
        context.write('mris_convert %s %s' %
                      (self.SphereReg.fullPath(), self.SphereRegGifti.fullPath()))
        launchFreesurferCommand(context, database,  'mris_convert',
                                self.SphereReg.fullPath(), self.SphereRegGifti.fullPath())
