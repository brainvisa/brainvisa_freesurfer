...
def initialization(self):
    ...
    # self.linkParameters('subject_tp', ('AnatImageTimepoint',
    # 'AnatImageTemplate'), linkSubjectNames)
...
def execution(self, context):
    ...
    if subject_tp is None:
        subject_tp = os.path.basename(
            os.path.dirname(os.path.dirname(os.path.dirname(self.AnatImageTimepoint.fullPath()))))
    subject_template = self.subject_template  # self.AnatImageTemplate.get('subject')
    context.write('Launch the Freesurfer longitudinal pipeline on subject timepoint ' +
                  subject_tp + ' template ' + subject_template)
    database = self.db.fullPath()  # self.AnatImageTimepoint.get('_database')
    if not database:
        database = os.path.dirname(
            os.path.dirname(os.path.dirname(os.path.dirname(self.AnatImage.fullPath()))))

    context.write('recon-all -long %s %s -all' %
                  (subject_template, subject_tp))
